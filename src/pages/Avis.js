  ...
  return (
    <div style={{ background: '#fff', fontFamily: sans, minHeight: '100vh' }}>
            ...
            <nav style={{ flex: 1, padding: '16px 0' }}>
              {MENU_LINKS.map(({ label, to }) => (
                <Link key={label} to={to} onClick={() => setMenuOpen(false)}
                  style={{ display: 'block', padding: '18px 40px', fontFamily: serif, fontSize: '1.5rem', fontWeight: 300, fontStyle: 'italic', color: to === '/avis' ? '#C9A84C' : '#1a1a1a', textDecoration: 'none', borderBottom: '.5px solid #f8f8f8', transition: 'color .2s' }}
                  onMouseEnter={e => e.currentTarget.style.color = '#C9A84C'}
                  onMouseLeave={e => e.currentTarget.style.color = to === '/avis' ? '#C9A84C' : '#1a1a1a'}
                >{label}</Link>
              ))}
            </nav>
            <div style={{ padding: '28px 40px', borderTop: '.5px solid #f0f0f0' }}>
              <Link to={demandeTo} onClick={() => setMenuOpen(false)}
                style={{ display: 'block', width: '100%', textAlign: 'center', fontFamily: sans, fontSize: '10px', letterSpacing: '.2em', textTransform: 'uppercase', color: '#fff', background: '#C9A84C', padding: '15px 0', textDecoration: 'none', boxSizing: 'border-box' }}
              ...
              <div style={{ position: 'absolute', top: '48px', right: 0, width: '220px', background: '#fff', border: '.5px solid #ececec', boxShadow: '0 8px 32px rgba(0,0,0,.08)', zIndex: 200 }}>
                {initials ? (
                  ...
                  <div style={{ padding: '8px 0' }}>
                    {[['Se connecter', '/login', 400], ['Créer un compte client', '/register/client', 300], ['Devenir expert', '/register/shopper', 300]].map(([l, t, w], i) => (
      ...
      <div style={{ padding: '140px 48px 64px', borderBottom: '.5px solid #ececec', boxSizing: 'border-box' }}>
        <p style={{ fontFamily: sans, fontSize: '10px', letterSpacing: '.28em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '12px' }}>540+ clients satisfaits</p>
        <h1 style={{ fontFamily: serif, fontSize: '4rem', fontWeight: 300, fontStyle: 'italic', color: '#1a1a1a', marginBottom: '14px', lineHeight: 1.1 }}>
          Ce que nos clients disent.
        </h1>
        <p style={{ fontFamily: serif, fontSize: '1.15rem', fontStyle: 'italic', color: '#aaa', fontWeight: 300, maxWidth: '520px' }}>
          Chaque avis correspond à une mission réelle, vérifiée et livrée par l'un de nos experts.
        </p>
      </div>

      {/* ── STATS ── */}
      <div style={{ background: '#1a1a1a', padding: '0 48px' }}>
        <div style={{ display: 'flex', maxWidth: '960px', margin: '0 auto' }}>
          {STATS.map(({ n, label }, i) => (
            <div key={label} style={{ flex: 1, padding: '24px', borderRight: i < 3 ? '.5px solid rgba(255,255,255,.06)' : 'none', textAlign: 'left' }}>
              <p style={{ fontFamily: serif, fontSize: '2rem', fontWeight: 300, fontStyle: 'italic', color: '#C9A84C', lineHeight: 1, marginBottom: '4px' }}>{n}</p>
              <p style={{ fontFamily: sans, fontSize: '9px', letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,.4)' }}>{label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '80px 48px', boxSizing: 'border-box' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', maxWidth: '1100px', margin: '0 auto' }}>
          {TEMOIGNAGES.map(({ photo, texte, nom, ville, piece, expert, etoiles }) => (
            <div key={nom} style={{ padding: '36px 32px', border: '.5px solid #e8e4dc', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              {/* Étoiles */}
              <div style={{ display: 'flex', gap: '3px' }}>
                {[1,2,3,4,5].map(i => <span key={i} style={{ color: '#C9A84C', fontSize: '13px' }}>★</span>)}
              </div>

              {/* Texte */}
              <p style={{ fontFamily: serif, fontSize: '1.05rem', fontStyle: 'italic', fontWeight: 300, color: '#333', lineHeight: 2, flex: 1 }}>"{texte}"</p>

              {/* Pièce trouvée */}
              <div style={{ background: '#fafaf8', padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                  <span style={{ fontFamily: sans, fontSize: '9px', color: '#555', letterSpacing: '.08em', fontWeight: 400 }}>{piece}</span>
                </div>
                <span style={{ fontFamily: sans, fontSize: '9px', color: '#bbb', paddingLeft: '16px' }}>Expert : {expert}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderTop: '.5px solid #ececec', paddingTop: '16px' }}>
                ...
                <div>
                  <p style={{ fontFamily: sans, fontSize: '11px', fontWeight: 500, color: '#1a1a1a', marginBottom: '2px' }}>{nom}</p>
                  <p style={{ fontFamily: sans, fontSize: '10px', color: '#bbb' }}>{ville}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* ── CTA ── */}
      <div style={{ padding: '64px 48px', background: '#fafaf8', borderTop: '.5px solid #ececec', boxSizing: 'border-box' }}>
        <p style={{ fontFamily: sans, fontSize: '10px', letterSpacing: '.28em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '14px' }}>Prêt à vous lancer ?</p>
        <h2 style={{ fontFamily: serif, fontSize: '2.4rem', fontWeight: 300, fontStyle: 'italic', color: '#1a1a1a', marginBottom: '14px' }}>
          Déposez votre demande gratuitement.
        </h2>
        <p style={{ fontFamily: serif, fontSize: '1.1rem', fontStyle: 'italic', color: '#aaa', fontWeight: 300, maxWidth: '440px', lineHeight: 1.9, marginBottom: '32px' }}>
